...
class MyBeds24 {
    ...
    makeOps(payload: { [key: string]: any }): { [key: string]: any } {
        return {
            method: "POST",
    ...
    getData(url: string, payload: { [key: string]: any }) {
        ...
        if (data.length >= 999) { throw new Error('1000を超えるデータは取得できません（APIの制限）。\nスクリプトを拡張してください') }
        return data;
    }
     ...
     */
    getBookings(modSince = new Date(), ops: { [key: string]: any }) {
        if (ops && ops.debug) {
            return ops.debug;
        }
        ops = Object.assign({
